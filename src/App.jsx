...
import { POOL, LOCKED_REGIONS, HIDDEN_REGIONS } from "./data/cards.js";
    ...
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 50, overflow: "hidden" }}>
      {effects.map((fx) => {
        if (fx.type === "damage") return (<Fragment key={fx.id}>
          ...
          {fx.amount > 0 && <div style={{ position:"absolute", top:"38%", left:"50%", transform:"translate(-50%,-50%)", animation:"vfxShake .3s cubic-bezier(0.25,0.46,0.45,0.94), vfxFloat 1s 0.08s cubic-bezier(0.22,0,0.36,1) forwards", fontSize:48, fontFamily:"'Cinzel',serif", fontWeight:900, color:"#ff2820", textShadow:"0 0 30px #ff0000ee, 0 0 60px #ff000066, 0 3px 6px rgba(0,0,0,0.95)", letterSpacing:1 }}>-{fx.amount}</div>}
        ...
        if (fx.type === "heal") return (<Fragment key={fx.id}>
          ...
          {fx.amount > 0 && <div style={{ position:"absolute", top:"38%", left:"50%", transform:"translate(-50%,-50%)", animation:"vfxFloat 1.1s cubic-bezier(0.22,0,0.36,1) forwards", fontSize:42, fontFamily:"'Cinzel',serif", fontWeight:900, color:"#38f068", textShadow:"0 0 24px #00ff44ee, 0 0 48px #00ff4466, 0 3px 6px rgba(0,0,0,0.95)", letterSpacing:1 }}>+{fx.amount}</div>}
        </Fragment>);
...
function CardV2({ card, size = "md", onClick }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      {/* ─── DARK VIGNETTE ─── */}
      <div style={{ position:"absolute", inset:0, pointerEvents:"none",
        background:"linear-gradient(to bottom, transparent 0%, transparent 22%, rgba(4,2,14,0.22) 38%, rgba(4,2,14,0.72) 52%, rgba(4,2,14,0.91) 63%, rgba(4,2,14,0.97) 73%, rgba(4,2,14,0.99) 100%)" }} />
      ...
      {/* ─── PANEL CONTENT — no bg, text-shadow carries readability ─── */}
      <div style={{
        ...
        <div style={{ display:"flex", alignItems:"center", gap:4, flexShrink:0 }}>
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{
              ...
              color:"#f5eba8", textTransform:"uppercase", letterSpacing:0.4, lineHeight:1.1,
              whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis",
              textShadow:"0 0 2px #000,0 0 5px #000,0 0 10px #000,0 1px 2px #000,0 2px 5px #000,0 3px 8px rgba(0,0,0,0.9)"
            }}>{card.name}</div>
            <div style={{
              fontSize: size==="sm" ? 6.5 : 7.5, color:"#b0a07a",
              fontStyle:"italic", fontFamily:"'Lora',serif", marginTop:1,
              textShadow:"0 0 4px #000,0 1px 3px #000,0 2px 6px rgba(0,0,0,0.9)"
        ...
        {/* Type line */}
        <div style={{
          fontSize: size==="sm" ? 6 : 7.5, color:"#908060",
          ...
          <div style={{ display:"flex", gap:4, flexWrap:"wrap", flexShrink:0 }}>
            {kws.map(k => (
              <span key={k.name} style={{
                ...
                background:"rgba(0,0,0,0.72)", border:`1px solid ${k.color}cc`,
                color:k.color, fontWeight:700, fontFamily:"'Cinzel',serif", letterSpacing:0.5,
                textShadow:`0 0 5px ${k.color}99, 0 0 2px #000`
              }}>{k.icon} {k.name}</span>
            ))}
          </div>
        )}
        ...
        <div style={{
          ...
          fontSize: size==="sm" ? 6.5 : 8, color:"#e0d0b0",
          lineHeight:1.55, fontFamily:"'Lora',serif",
          textShadow:"0 0 3px #000,0 0 6px #000,0 1px 3px #000,0 2px 5px rgba(0,0,0,0.9)"
        }}>{card.ability}</div>

        {/* Bottom metadata */}
        <div style={{
          display:"flex", justifyContent:"space-between", alignItems:"center",
          flexShrink:0, fontSize:5.5, color:"#6a5038",
...
const ALT_ARTS = {
  wolf:       [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Common",    imageUrl:"/alt-art/stonefang-anime-island.webp", rarity:"Common" }],
  guard:      [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Common",    imageUrl:"/alt-art/guard-anime-island.webp",     rarity:"Common" }],
  druid:      [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Uncommon",  imageUrl:"/alt-art/druid-anime-island.webp",     rarity:"Uncommon" }],
  tangle:     [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Rare",      imageUrl:"/alt-art/tangle-anime-island.webp",    rarity:"Rare" }],
  env_grove:  [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Uncommon",  imageUrl:"/alt-art/env_grove-anime-island.webp", rarity:"Uncommon" }],
  wisp:       [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Uncommon",  imageUrl:"/alt-art/wisp-anime-island.webp",      rarity:"Uncommon" }],
  shard:      [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Common",    imageUrl:"/alt-art/shard-anime-island.webp",     rarity:"Common" }],
  weaver:     [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Rare",      imageUrl:"/alt-art/weaver-anime-island.webp",    rarity:"Rare" }],
  velrun:     [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Legendary", imageUrl:"/alt-art/velrun-anime-island.webp",    rarity:"Legendary", freeForOwners: true, altObjectPosition: "50% 15%" }],
  env_rift:   [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Rare",      imageUrl:"/alt-art/env_rift-anime-island.webp",  rarity:"Rare" }],
  tide:       [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Rare",      imageUrl:"/alt-art/tide-anime-island.webp",      rarity:"Rare" }],
  shellguard: [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Common",    imageUrl:"/alt-art/shellguard-anime-island.webp",rarity:"Common" }],
  current:    [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Common",    imageUrl:"/alt-art/current-anime-island.webp",   rarity:"Common" }],
  kraken:     [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Epic",      imageUrl:"/alt-art/kraken-anime-island.webp",    rarity:"Epic" }],
  env_depths: [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Uncommon",  imageUrl:"/alt-art/env_depths-anime-island.webp",rarity:"Uncommon" }],
  sprite:     [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Common",    imageUrl:"/alt-art/sprite-anime-island.webp",    rarity:"Common" }],
  imp:        [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Common",    imageUrl:"/alt-art/imp-anime-island.webp",       rarity:"Common" }],
  pyro:       [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Uncommon",  imageUrl:"/alt-art/pyro-anime-island.webp",      rarity:"Uncommon" }],
  eruption:   [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Rare",      imageUrl:"/alt-art/eruption-anime-island.webp",  rarity:"Rare" }],
  env_volcano:[{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Rare",      imageUrl:"/alt-art/env_volcano-anime-island.webp",rarity:"Rare" }],
  sentinel:   [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Uncommon",  imageUrl:"/alt-art/sentinel-anime-island.webp",  rarity:"Uncommon" }],
  forgebot:   [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Common",    imageUrl:"/alt-art/forgebot-anime-island.webp",  rarity:"Common" }],
  shield_wall:[{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Common",    imageUrl:"/alt-art/shield_wall-anime-island.webp",rarity:"Common" }],
  colossus:   [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Legendary", imageUrl:"/alt-art/colossus-anime-island.webp",  rarity:"Legendary" }],
  falcon:     [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Common",    imageUrl:"/alt-art/falcon-anime-island.webp",    rarity:"Common" }],
  oracle:     [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Uncommon",  imageUrl:"/alt-art/oracle-anime-island.webp",    rarity:"Uncommon" }],
  sun_strike: [
    { setId:"anime_island", setName:"Anime Island", label:"Anime Island · Rare",      imageUrl:"/alt-art/sun_strike-anime-island.webp",  rarity:"Rare" },
    { setId:"prismatic",    setName:"Prismatic",    label:"☀ Prismatic Sun Strike ✦", imageUrl:"/alt-art/sun_strike2-anime-island.webp", rarity:"Prismatic" },
  ],
  env_dunes:  [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Uncommon",  imageUrl:"/alt-art/env_dunes-anime-island.webp", rarity:"Uncommon" }],
  siphon:     [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Rare",      imageUrl:"/alt-art/siphon-anime-island.webp",    rarity:"Rare" }],
  martyr:     [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Uncommon",  imageUrl:"/alt-art/martyr-anime-island.webp",    rarity:"Uncommon" }],
  bloodmage:  [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Epic",      imageUrl:"/alt-art/bloodmage-anime-island.webp", rarity:"Epic" }],
  blood_pact: [{ setId:"anime_island", setName:"Anime Island", label:"Anime Island · Uncommon",  imageUrl:"/alt-art/blood_pact-anime-island.webp",rarity:"Uncommon" }],
};
...
// ═══ ENEMY AI ════════════════════════════════════════════════════════════════
function scoreThreat(c) {
  const kws = c.keywords || [];
  let bonus = 0;
  if (kws.includes("Haunt"))   bonus += 4;
  if (kws.includes("Resonate"))bonus += 3;
  if (kws.includes("Charge"))  bonus += 3;
  if (kws.includes("Echo"))    bonus += 2;
  if (kws.includes("Anchor"))  bonus += 2;
  if (kws.includes("Bleed"))   bonus += 2;
  if (kws.includes("Swift"))   bonus += 1;
  return c.currentAtk * 3 + c.currentHp + bonus;
}
...
function computeEnemyPlayPhase(g, vfx) {
  ...
  let en = s.maxEnergy;
  [...s.enemyHand].sort((a, b) => { const sp = c => c.type === "spell" && s.playerBoard.length >= 3 ? 1 : 0; if (sp(b) !== sp(a)) return sp(b) - sp(a); return b.cost - a.cost; }).forEach((card) => {
...
function computeEnemyAttackPhase(g, vfx) {
  ...
  s.enemyBoard.filter((c) => c.canAttack && !c.hasAttacked && (!(c.keywords||[]).includes("Charge") || (c.chargeCount||0) > 0)).forEach((att) => {
    ...
    if (s.playerBoard.length > 0) { const readyAtk = s.enemyBoard.filter(c => !c.hasAttacked && c.canAttack && (!(c.keywords||[]).includes("Charge") || (c.chargeCount||0) > 0)).reduce((sum, c) => sum + c.currentAtk, 0); if (readyAtk >= s.playerHP) { s.playerHP -= av; s.enemyBoard = s.enemyBoard.map((c) => c.uid === att.uid ? { ...c, hasAttacked: true, chargeCount: (c.keywords||[]).includes("Charge") && c.chargeCount != null ? Math.max(0, c.chargeCount - 1) : c.chargeCount } : c); L(`${att.name} rushes face for ${av}!`); if (s.enemyZeusInPlay && (att.keywords || []).includes("Swift")) { s.enemyLightningMeter = (s.enemyLightningMeter || 0) + 1; if (s.enemyLightningMeter >= 2) { s = fireLightningMeter(s, "enemy", vfx, L); } } s = resolveEffects("onAttack", att, s, "enemy", vfx); } else { const kills = s.playerBoard.filter(t => (t.shielded ? t.currentHp : t.currentHp - av) <= 0); const cleanKills = kills.filter(t => !((att.shielded ? att.currentHp : att.currentHp - t.currentAtk) <= 0)); const tgt = cleanKills.length > 0 ? cleanKills.sort((a, b) => scoreThreat(b) - scoreThreat(a))[0] : kills.length > 0 ? kills.sort((a, b) => scoreThreat(b) - scoreThreat(a))[0] : [...s.playerBoard].sort((a, b) => scoreThreat(b) - scoreThreat(a))[0]; let nTHP = tgt.shielded ? tgt.currentHp : tgt.currentHp - av; let nAHP = att.shielded ? att.currentHp : att.currentHp - tgt.currentAtk; if (tgt.shielded) L(`${tgt.name} shield absorbs!`); if (att.shielded) L(`${att.name} shield absorbs counter!`); s.enemyBoard = s.enemyBoard.map((c) => c.uid === att.uid ? { ...c, hasAttacked: true, currentHp: nAHP, shielded: false, chargeCount: (c.keywords||[]).includes("Charge") && c.chargeCount != null ? Math.max(0, c.chargeCount - 1) : c.chargeCount } : c).filter((c) => c.currentHp > 0); s.playerBoard = s.playerBoard.map((c) => c.uid === tgt.uid ? { ...c, currentHp: nTHP, shielded: false, bleed: (c.bleed || 0) + ((att.keywords || []).includes("Bleed") ? (att.bleedAmount || 1) : 0) } : c).filter((c) => c.currentHp > 0); if (nTHP <= 0) { L(`${tgt.name} falls!`); s = resolveEffects("onDeath", tgt, s, "player", vfx); if (s.playerBoard.find(c => c.id === "hades_soul_reaper") || s.playerHand.find(c => c.id === "hades_soul_reaper")) { s = resolveEffects("onFriendlyDeath", {id:"hades_soul_reaper",effects:[{trigger:"onFriendlyDeath",effect:"soul_harvest"}]}, s, "player", vfx); } } if (nAHP <= 0) s = resolveEffects("onDeath", att, s, "enemy", vfx); }
...
function BattleScreen({ user, onUpdateUser, matchConfig, onExit }) {
  ...
  const playCard = (card, targetUid = null) => {
    ...
    if (card.type === "spell") {
      ...
      setGame((prev) => {
        ...
        if (s.playerZeusInPlay) { s.playerLightningMeter = (s.playerLightningMeter||0) + 1; if (s.playerLightningMeter >= 2) { s = fireLightningMeter(s, "player", vfx, (m) => { s.log = [...s.log.slice(-20), m]; }); } }
  ...
  return (<div className="battle-wrapper" style={{ width:"100%", height:"calc(100vh - 72px)", padding:"8px 14px 6px", background:"radial-gradient(ellipse at 50% -5%, #1e1a0e 0%, #141008 35%, #0c0a04 70%, #080600 100%)", boxSizing:"border-box", overflow:"visible", display:"flex", flexDirection:"column" }} onClick={() => { SFX.init(); }}>
    ...
    <div className="battle-grid" style={{ display:"grid", gridTemplateColumns:"280px 1fr 300px", gap:14, flex:1, minHeight:0 }}>
      ...
      <div style={{ background: envTheme ? envTheme.bg : "linear-gradient(180deg,#1e1a0e 0%,#141008 50%,#0e0c06 100%)", border: `1px solid ${envTheme ? envTheme.glow + "44" : "rgba(200,144,16,0.18)"}`, borderRadius: 14, overflow: "visible", position: "relative", transition: "background 1.5s ease, border-color 1s ease", boxShadow: envTheme ? undefined : "inset 0 0 80px rgba(0,0,0,0.55), 0 0 0 1px rgba(200,144,16,0.08), 0 0 60px rgba(20,14,0,0.12)", display:"flex", flexDirection:"column", height:"100%" }}>
        ...
        <div style={{ padding: "3px 14px", background: envTheme ? "rgba(0,0,0,0.45)" : "rgba(8,6,2,0.9)", borderBottom: "2px solid rgba(200,144,16,0.22)", borderTop: "2px solid rgba(200,144,16,0.22)", display: "flex", alignItems: "center", gap: 10, position: "relative", zIndex: 2 }}>
...
function PvpBattleScreen({ user, matchConfig, onExit, onUpdateUser, setInPvpMatch }) {
  ...
  const applyPvpAction = (gs, action, role, vfxInst) => {
    ...
    if (action.type === "play_card") {
      ...
      if (ai.playerZeusInPlay && card.type === "spell") { ai.playerLightningMeter = (ai.playerLightningMeter||0) + 1; if (ai.playerLightningMeter >= 2) { const pvpLog = []; ai = fireLightningMeter(ai, "player", null, m => pvpLog.push(m)); ai.log = [...ai.log.slice(-20), ...pvpLog]; } }
...
// ═══ COLLECTION ══════════════════════════════════════════════════════════════
const CRAFT_COST    = { Common:40, Uncommon:100, Rare:250, Epic:600, Legendary:1500 };
const DISENCHANT_VAL= { Common:5,  Uncommon:15,  Rare:40,  Epic:100, Legendary:250  };
...
function CollectionScreen({ user, onUpdateUser, onDeckBuilding, newPlayerMode }) {
  ...
  const [craftPending, setCraftPending] = useState(null);

  const craftCard = async (card) => {
    const cost = CRAFT_COST[card.rarity] || 9999;
    if ((user?.shards || 0) < cost) return;
    const newCol = { ...(user.collection || {}), [card.id]: (col[card.id] || 0) + 1 };
    await onUpdateUser({ shards: (user.shards || 0) - cost, collection: newCol });
    setCraftPending(null);
    SFX.play("card_flip");
  };
  const disenchantCard = async (card) => {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px 60px" }} onClick={() => { if(artPicker) setArtPicker(null); }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          {/* Craft Mode Toggle */}
          <button onClick={() => { setCraftMode(m => !m); setCraftPending(null); }}
            style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 18px", background: craftMode ? "linear-gradient(135deg,#1a0c38,#0a0820)" : "linear-gradient(135deg,#0c1428,#060e20)", border:`2px solid ${craftMode?"#60a0e0aa":"#3060a055"}`, borderRadius:12, cursor:"pointer", fontFamily:"'Cinzel',serif", color: craftMode ? "#80c8f0" : "#4080b0", transition:"all .2s" }}>
            <span style={{ fontSize:18 }}>⬙</span>
            <div style={{ textAlign:"left" }}>
              <div style={{ fontSize:11, fontWeight:700, letterSpacing:1 }}>{craftMode ? "EXIT CRAFT" : "CRAFT"}</div>
              <div style={{ fontSize:8, color: craftMode?"#4080a0":"#305070", marginTop:1 }}>forge cards from shards</div>
            </div>
          </button>
...
const TUT_SQUIRE = { id:"tut_sq", name:"Thornwood Guard", type:"creature", cost:2, atk:2, hp:4, keywords:[], border:"#4a9020", imageUrl:"/cards/guard.webp", imageScale:1.1, ability:"On Play: Give +1 HP to all allies.", region:"Thornwood", rarity:"Common", uid:"tut_sq1", currentHp:4, maxHp:4, currentAtk:2, canAttack:false, hasAttacked:false, bleed:0 };
const TUT_FILLER1 = { id:"tut_f1", name:"Rootcaller Druid", type:"creature", cost:3, atk:2, hp:3, keywords:[], border:"#4a9020", imageUrl:"/cards/druid.webp", imageScale:1.1, ability:"On Play: Heal hero for 3.", region:"Thornwood", rarity:"Uncommon", uid:"tut_f1", currentHp:3, maxHp:3, currentAtk:2, canAttack:false, hasAttacked:false, bleed:0 };
const TUT_FILLER2 = { id:"tut_f2", name:"Tanglewood Trap", type:"spell", cost:2, atk:null, hp:null, keywords:[], border:"#4a9020", imageUrl:"/cards/tangle.webp", imageScale:1.1, ability:"Deal 2 damage to all enemies.", region:"Thornwood", rarity:"Rare", uid:"tut_f2" };
const TUT_ENEMY_CARD = { id:"tut_en", name:"Echo Wisp", type:"creature", cost:2, atk:2, hp:2, keywords:["Echo"], border:"#9050d8", imageUrl:"/cards/wisp.webp", imageScale:1.1, ability:"Echo — a 1/1 ghost replays next turn.", region:"Shattered Expanse", rarity:"Uncommon", uid:"tut_en1", currentHp:2, maxHp:2, currentAtk:2, canAttack:false, hasAttacked:false, bleed:0 };
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap", marginBottom: 28, minHeight: 280 }}>
            {opening.cards.map((card, i) => {
              ...
              return (
                <div key={i} onClick={!isRevealed ? revealNext : undefined} style={{ width: 142, cursor: isRevealed ? "default" : "pointer", perspective: 1000 }}>
                  <div style={{ transition: "transform 0.55s cubic-bezier(.4,0,.2,1)", transformStyle: "preserve-3d", transform: isRevealed ? "rotateY(0deg)" : isShaking ? "rotateY(90deg)" : "rotateY(180deg)", willChange: "transform", backfaceVisibility: "hidden" }}>
                    {isRevealed ? (
                      <div style={{ animation: "cardReveal 0.5s ease-out", position: "relative" }}>
                        {rarGlow && <div style={{ position: "absolute", inset: -8, borderRadius: 20, background: `radial-gradient(circle,${rarGlow}33,transparent 70%)`, animation: "vfxPulse 1s ease-out", pointerEvents: "none", zIndex: -1 }} />}
...
export default function App() {
  ...
  return (<div style={{ minHeight: "100vh", background: "#080310", color: "#e8e0d0", fontFamily: "'Lora',Georgia,serif", overflowX: "hidden" }} onClick={() => setShowSidebar(false)}>
    ...
    <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, background: "radial-gradient(ellipse at 20% 10%,rgba(180,100,10,0.10) 0%,transparent 50%),radial-gradient(ellipse at 80% 90%,rgba(100,50,10,0.08) 0%,transparent 50%),radial-gradient(ellipse at 50% 50%,rgba(160,80,0,0.05) 0%,transparent 60%)" }} />

    <nav style={{ position: "sticky", width: "100%", top: 0, zIndex: 100, background: "linear-gradient(180deg,#1e1a0e 0%,#14100a 100%)", borderBottom: "2px solid rgba(200,150,16,0.28)", padding: "0 20px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 72, boxShadow: "0 4px 40px rgba(0,0,0,0.85), 0 1px 0 rgba(232,192,96,0.10)", overflow: "hidden", WebkitFontSmoothing: "antialiased" }} onClick={(e) => { e.stopPropagation(); }}>
...
>
              ...
              <span className="nav-labels" style={{ fontFamily: "'Cinzel',serif", fontSize: 10, fontWeight: 700, color: active ? "#e8c060" : "#605038", letterSpacing: 1, lineHeight: 1, transition: "color .16s ease" }}>{t.label}</span>
            </button>
          );
        })}
      </div>
